#!/usr/bin/env python3
"""
Upload Google Places Phase 3 Data to BigQuery
"""
...
def main():
    # Load Phase 3 data
    df = pd.read_csv('google_places_phase3_20250627_215651.csv')
    logger.info(f"Loaded {len(df)} Phase 3 records")
    ...
    # Use same table as Phase 1 & 2 to combine all data
    ...
    logger.info("Uploading Phase 3 data to BigQuery...")
    job = client.load_table_from_dataframe(df_clean, table_id, job_config=job_config)
    job.result()
    
    # Verify total count
    table = client.get_table(table_id)
    logger.info(f"✅ SUCCESS! Phase 3 uploaded")
    logger.info(f"Phase 3 records: {len(df_clean)}")
